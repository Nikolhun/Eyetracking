...
def get_threshold_left(event):
    ...
    while cam.isOpened():
        ...
        for face in faces:
            ...
            landmarks = predictor_dlib(gray, face)  # detect face structures using landmarks
            left_landmarks_array = landmarks_array(36, 37, 38, 39, 40, 41, landmarks, gray, lines=0)
            eye_fill = fill_frame_gui(gray, left_landmarks_array)
            left_eye_crop, min_left = crop_eyes(eye_fill, left_landmarks_array)
            no_reflex_left = detect_corneal_reflection(left_eye_crop, threshold_left_scale.get())
            ...
            eye_preprocessed_left = preprocessing(gama_corrected_left, threshold_left_scale.get())
            cv2.imshow("eye_processed_left", eye_preprocessed_left)
            contours_left = contours_of_shape(eye_preprocessed_left, threshold_left_scale.get())
...
def get_threshold_right(event):
    ...
    while cam.isOpened():
        ...
        for face in faces:
            ...
            landmarks = predictor_dlib(gray, face)  # detect face structures using landmarks
            right_landmarks_array = landmarks_array(42, 43, 44, 45, 46, 47, landmarks, gray, lines=0)
            eye_fill = fill_frame_gui(gray, right_landmarks_array)
            right_eye_crop, min_right = crop_eyes(eye_fill, right_landmarks_array)
            no_reflex_right = detect_corneal_reflection(right_eye_crop, threshold_right_scale.get())
            ...
            eye_preprocessed_right = preprocessing(gama_corrected_right, threshold_right_scale.get())
            cv2.imshow("preprocessing_right", eye_preprocessed_right)
            contours_right = contours_of_shape(eye_preprocessed_right, threshold_right_scale.get())
